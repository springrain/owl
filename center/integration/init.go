...
func Init(ctx *ctx.Context, builtinIntegrationsDir string) {
	...
	for _, dir := range dirList {
		...
		f1 := zorm.NewFinder().Append("delete from builtin_payloads where uuid = 0 and type != 'collect' and (updated_by = 'system' or updated_by = '')")
		f1.InjectionCheck = false
		err = models.UpdateFinder(ctx, f1)
		...
		f2 := zorm.NewFinder().Append("delete from builtin_metrics where uuid = 0 and (updated_by = 'system' or updated_by = '')")
		f2.InjectionCheck = false
		err = models.UpdateFinder(ctx, f2)
