...
func (s *Scheduler) syncAlertRules() {
	...
	for _, id := range ids {
		...
		if rule.IsPrometheusRule() || rule.IsLokiRule() || rule.IsTdengineRule() {
			datasourceIds := s.datasourceCache.GetIDsByDsCateAndQueries(rule.Cate, rule.DatasourceQueriesJson)
			...
			dsIds := s.datasourceCache.GetIDsByDsCateAndQueries(rule.Cate, rule.DatasourceQueriesJson)
